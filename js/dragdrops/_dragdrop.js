inject.define("dragdrops.dragdrop", [
    ...
    function (dao, legend) {
        ...
        var dropComponentProjectLayout = function(event, ui) {
            var $this = $(ui.draggable);
            if (!$this.hasClass('capsule')) {
                console.debug('COMPONENTE NÃO POSSUI A CLASS capsule [return]');
                return;
            }
            ...
            sortable(['.des-container', '.des-datasource', '.capsule']);

            draggable($('#palleta .component'), ['#project .capsule'],
                function(event, ui) {                   
                    //ui.helper.width(450);
                });

            droppable(['.capsule'], dropComponent);
        };

        self.dragdrop = function() {
            sortable(['.des-container', '.des-datasource', '.capsule']);

            draggable($('#palleta .capsule'), ['#project.des-container'], 
                function(event, ui) {
                    //ui.helper.width(450)
                });
